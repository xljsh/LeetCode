...
class Cluster
{
  ...
  float length_, width_, height_, long_side_, short_side_;
...
public:
  ...
  std::vector<double> GetOrientationQuaternion();
  /* \brief Returns the Length of the Cluster */
  double GetEstimatedSlopAngle();
  float GetLenght();
  ...
  void SetROSHeader(std_msgs::Header in_ros_header);
  ...
  void AddCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud_ptr, const std::vector<int>& in_cluster_indices, bool optimizor);
  void LShapeOptimize();
